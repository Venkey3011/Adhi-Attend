...
// MongoDB Connection
const MONGODB_URI = process.env.MONGODB_URI || "mongodb+srv://admin:admin123@ac-vhxwydf.nesqvfa.mongodb.net/Attendance?retryWrites=true&w=majority";
...
app.post("/api/users", authenticate, isAdmin, async (req, res) => {
  const { username, password, role, name, department, permissions } = req.body;
  try {
    const hashedPassword = await bcrypt.hash(password, 10);
    const user = new User({ username, password: hashedPassword, role, name, department, permissions });
    await user.save();
...
app.put("/api/users/:id", authenticate, isAdmin, async (req, res) => {
  try {
    const { username, password, role, name, department, permissions } = req.body;
    const updateData: any = { username, role, name, department, permissions };
    if (password) {
...
app.get("/api/attendance-types", authenticate, async (req, res) => {
  try {
    const types = await AttendanceType.find().sort({ name: 1 });
    res.json(types);
  } catch (err) {
...
app.post("/api/attendance-types", authenticate, isAdmin, async (req, res) => {
  try {
    const { name } = req.body;
    if (!name) return res.status(400).json({ error: "Name is required" });
    const type = new AttendanceType({ name });
    await type.save();
    res.json(type);
  } catch (err: any) {
    if (err.code === 11000) return res.status(400).json({ error: "Type already exists" });
    res.status(500).json({ error: "Failed to create attendance type" });
  }
});

app.delete("/api/attendance-types/:id", authenticate, isAdmin, async (req, res) => {
  try {
    await AttendanceType.findByIdAndDelete(req.params.id);
    res.json({ message: "Deleted successfully" });
  } catch (err) {
    res.status(500).json({ error: "Failed to delete attendance type" });
  }
});
...
app.post("/api/auth/login", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: user._id, role: user.role, name: user.name, department: user.department, permissions: user.permissions }, JWT_SECRET);
    res.json({ token, user: { id: user._id, role: user.role, name: user.name, department: user.department, permissions: user.permissions } });
  } catch (err) {
    res.status(500).json({ error: "Server error" });
  }
});
...
// Student Routes
app.get("/api/students/template", authenticate, hasPermission('manageStudents'), (req, res) => {
  const templateData = [
    {
      Name: "John Doe",
      RollNumber: "21CS001",
...
app.post("/api/students", authenticate, hasPermission('manageStudents'), async (req, res) => {
  try {
...
app.put("/api/students/:id", authenticate, hasPermission('manageStudents'), async (req, res) => {
  try {
...
app.delete("/api/students/:id", authenticate, hasPermission('manageStudents'), async (req, res) => {
  try {
...
app.post("/api/students/bulk-delete", authenticate, hasPermission('manageStudents'), async (req, res) => {
  try {
    const { ids } = req.body;
...
app.post("/api/students/bulk", authenticate, hasPermission('manageStudents'), upload.single("file"), async (req: any, res) => {
...
app.post("/api/attendance/session", authenticate, hasPermission('markAttendance'), async (req, res) => {
  let { type, batch, department, records, sessionId } = req.body;
  console.log("Submitting attendance:", { type, batch, department, sessionId, recordsCount: records.length });
  try {
    if (!sessionId) {
      // Double check if a session already exists for today to prevent duplicates
    ...
    if (sessionId) {
      ...
      res.json({ message: "Attendance updated successfully", sessionId });
    } else {
      // Create new
      ...
      await AttendanceRecord.insertMany(attendanceRecords);
      res.json({ message: "Attendance marked successfully", sessionId: session._id });
    }
  } catch (err) {
...
app.get("/api/attendance/reports", authenticate, hasPermission('reports'), async (req, res) => {
...
app.put("/api/attendance/records/:id", authenticate, hasPermission('reports'), async (req, res) => {
  try {
...
// Export Route
app.get("/api/attendance/export", authenticate, hasPermission('reports'), async (req, res) => {
  const { startDate, endDate, batch, department, type } = req.query;
  const filter: any = {};
...
app.get("/api/stats/department", authenticate, async (req, res) => {
  try {
    const { startDate, endDate, batch, department } = req.query;
    ...
    if (startDate || endDate) {
      ...
      if (startDate && startDate !== "") {
        const start = new Date(startDate as string);
        start.setUTCHours(0, 0, 0, 0);
        sessionFilter.date.$gte = start;
      }
      if (endDate && endDate !== "") {
        const end = new Date(endDate as string);
        end.setUTCHours(23, 59, 59, 999);
        sessionFilter.date.$lte = end;
      }
      if (Object.keys(sessionFilter.date).length === 0) delete sessionFilter.date;
    } else {
      ...
      const tomorrow = new Date(today);
      tomorrow.setDate(tomorrow.getDate() + 1);
      sessionFilter.date = { $gte: today, $lt: tomorrow };
    }

    if (batch && batch !== "") sessionFilter.batch = batch;
    ...
    // Get total students per batch from the Student collection
    const allBatches = await Student.distinct("batch");
    const batchTotals: any = {};
    for (const b of allBatches) {
      const totalFilter: any = { batch: b };
      if (sessionFilter.department) {
        totalFilter.department = sessionFilter.department;
      }
      batchTotals[b] = await Student.countDocuments(totalFilter);
    }
    ...
    for (const session of sessions) {
      ...
      if (!batchStats[b]) {
        batchStats[b] = { departments: {}, totalStudents: batchTotals[b] || 0 };
      }
      
      if (!batchStats[b].departments[dept]) {
        batchStats[b].departments[dept] = { present: 0, absent: 0, od: 0, total: 0 };
      }
      ...
      batchStats[b].departments[dept].present += p;
      batchStats[b].departments[dept].absent += a;
      batchStats[b].departments[dept].od += od;
      batchStats[b].departments[dept].total += records.length;
    }
    ...
    if (MONGODB_URI) {
      mongoose.connection.asPromise().then(async () => {
        try {
          ...
          const typeCount = await AttendanceType.countDocuments();
          if (typeCount === 0) {
            const initialTypes = ['Java Class', 'HackerRank Test', 'Oracle Training', 'Soft Skills', 'Others'];
            await AttendanceType.insertMany(initialTypes.map(name => ({ name })));
            console.log("✅ Initial attendance types seeded");
          }
        } catch (err) {
